...
| `ALLOWED_ORIGINS` | empty | Unified origin allowlist (comma-separated) used by both CORS and WebSocket checks; fallback order is `ALLOWED_ORIGINS` → `CORS_ALLOWED_ORIGINS` → `FRONTEND_ORIGIN` → `localhost:3000/5173/5174` |
...
<Callout type="info">
If you set both `FRONTEND_ORIGIN` and `CORS_ALLOWED_ORIGINS`, include `FRONTEND_ORIGIN` in `CORS_ALLOWED_ORIGINS`. If you also set `ALLOWED_ORIGINS`, include it there too to keep browser CORS and WebSocket Origin checks aligned.
</Callout>
